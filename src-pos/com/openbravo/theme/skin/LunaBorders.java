        ...
        public void paintBorder(Component c, Graphics g, int x, int y, int w, int h) {
            ...
            if (button.getRootPane() != null && button.equals(button.getRootPane().getDefaultButton())) {
                if (!button.getModel().isRollover()) {
                    ...
                    g2D.drawRoundRect(x + 2, y + 2, w - 6, h - 7, 6, 6);
                    g2D.setColor(defaultColorLo);
                    g2D.drawLine(x + 3, h - 3, w - 3, h - 3);
                    g2D.drawLine(w - 3, y + 4, w - 3, h - 3);
                }
            }

    //set the BORDER EDGESSSSS edges of an buttoonn
            //button edges with curvy in sales screen and left buttons of number keys---ABHILASH
            g2D.setColor(Color.GRAY);
            g2D.drawRoundRect(x, y, w - 1, h - 1, 3, 3);


            g2D.setColor(frameColor);
            g2D.drawRoundRect(x, y, w - 1, h - 1, 0, 0);
