...
public class JRetailRefundLines extends javax.swing.JPanel {
    
    private JRetailTicketLines ticketlines;
    private List m_aLines;

    private JRetailPanelTicketEdits m_jTicketEdit;
    
    /** Creates new form JRefundLines */
public JRetailRefundLines(DataLogicSystem dlSystem, JRetailPanelTicketEdits jTicketEdit) {
        
        m_jTicketEdit = jTicketEdit;
        
        initComponents();
        
        ticketlines = new JRetailTicketLines(dlSystem.getResourceAsXML("Ticket.Line"));
    ...
    public void setLines(List aRefundLines) {
        ...
        if (m_aLines != null) {
            for (int i = 0; i < m_aLines.size(); i++) {
                ticketlines.addTicketLine((RetailTicketLineInfo) m_aLines.get(i));
            }
        }
    }
    ...
    private void m_jbtnAddAllActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_m_jbtnAddAllActionPerformed
        System.out.println("entreee add all action");
        for (int i = 0; i < m_aLines.size(); i++) {
            RetailTicketLineInfo oLine = (RetailTicketLineInfo) m_aLines.get(i);
            RetailTicketLineInfo oNewLine = new RetailTicketLineInfo(oLine);
            oNewLine.setTaxValue(-oLine.getTaxRate());
            oNewLine.setMultiply(-oLine.getMultiply());
            oNewLine.setDiscount(-oLine.getDiscount());
            oNewLine.setTicketType(1);
    ...
    private void m_jbtnAddOneActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_m_jbtnAddOneActionPerformed
        ...
        if (index >= 0) {
            RetailTicketLineInfo oLine = (RetailTicketLineInfo) m_aLines.get(index);
            RetailTicketLineInfo oNewLine = new RetailTicketLineInfo(oLine);
            oNewLine.setTaxValue(oNewLine.getTaxRate());
