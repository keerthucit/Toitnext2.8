 ...
 * @author mateen
 */
public class JGoodsReturnLines  extends javax.swing.JPanel {
    ...
    public JGoodsReturnLines() {
        ...
        columns.addColumn(c);
        c = new TableColumn(1, 180, new DataCellRenderer(javax.swing.SwingConstants.LEFT), new DefaultCellEditor(new JTextField()));
        c.setHeaderValue(AppLocal.getIntString("label.item"));
        columns.addColumn(c);
        c = new TableColumn(2, 45, new DataCellRenderer(javax.swing.SwingConstants.CENTER), new DefaultCellEditor(new JTextField()));
        c.setHeaderValue(AppLocal.getIntString("label.units"));
        columns.addColumn(c);
         c = new TableColumn(3, 45, new DataCellRenderer(javax.swing.SwingConstants.CENTER), new DefaultCellEditor(new JTextField()));
        ...
        columns.addColumn(c);
        c = new TableColumn(5, 80, new DataCellRenderer(javax.swing.SwingConstants.CENTER), new DefaultCellEditor(new JTextField()));
    ...
    public void addLine(GoodsReturnLine i) {
        m_inventorylines.addRow(i);

        setSelectedIndex(m_inventorylines.getRowCount() - 1);
    }

    public void updateLine(GoodsReturnLine i, int row) {
    ...
    public void setLine(int index, GoodsReturnLine i) {
        m_inventorylines.setRow(index, i);
        setSelectedIndex(index);
    }

    public GoodsReturnLine getLine(int index) {
    ...
    public static void setMultiplychanged(boolean multiplychanged) {
        ...
        double taxtotal = 0.0;
        if (multiplychanged) {
            java.util.List<GoodsReturnLine> lines = m_inventorylines.getLines();
            for (GoodsReturnLine l : lines) {
            ...
            JGoodsReturn.setChangesmade(multiplychanged);
            JGoodsReturn.setTotals(total, subtotal, taxtotal);
    ...
    private static class InventoryTableModel extends AbstractTableModel {
        ...
        private ArrayList<GoodsReturnLine> m_rows = new ArrayList<GoodsReturnLine>();
        ...
        public Object getValueAt(int row, int column) {
            ...
            GoodsReturnLine i = m_rows.get(row);
            switch (column) {
        ...
        public void setValueAt(Object o, int row, int column) {
            ...
            if (column == 2) {
                ...
                if (o.toString().startsWith("x")) {
                    ...
                    double dMul = Integer.parseInt(trimStr);
                    i.setMultiply(dMul);
                    if (dMul <= 0) {
                    ...
                    try {
                        double dMul = Integer.parseInt(o.toString());
                        i.setMultiply(dMul);
                        if (dMul <= 0) {
                            removeRowFromPanelAndDatabase(row);
                            m_inventorylines.removeRow(row);
        ...
        public GoodsReturnLine getRow(int index) {
            return m_rows.get(index);
        }

        public void setRow(int index, GoodsReturnLine oLine) {

            m_rows.set(index, oLine);
            fireTableRowsUpdated(index, index);
        }

        public void addRow(GoodsReturnLine oLine) {

            insertRow(m_rows.size(), oLine);
        }

        public void insertRow(int index, GoodsReturnLine oLine) {
    ...
    private static void removeRowFromPanelAndDatabase(int row) {
        GoodsReturnLine line = m_inventorylines.getRow(row);
        JGoodsReturn.removeLineFromDB(line);
    }
