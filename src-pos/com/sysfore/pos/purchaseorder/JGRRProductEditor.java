 ...
 * @author mateen
 */
public class JGRRProductEditor extends javax.swing.JDialog {

    private GoodsReturnProductInfo m_ProdInfo;
    private boolean m_bunitsok;
    private boolean m_bpriceok;
    private GoodsReturnLine oLine;
    private AppView app;
    private double rate;
     ...
     */
    private JGRRProductEditor(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
    }
     ...
     * Creates new form JProductLineEdit
     */
    private JGRRProductEditor(java.awt.Dialog parent, boolean modal) {
        super(parent, modal);
    }

    private GoodsReturnLine init(AppView app, GoodsReturnLine oLine, int i) throws BasicException {
        ...
        m_jName.setEnabled(!app.getAppUserView().getUser().hasPermission("com.sysfore.pos.purchaseorder.JGoodsReturn"));
        m_jPrice.setEnabled(app.getAppUserView().getUser().hasPermission("com.sysfore.pos.purchaseorder.JGoodsReturn"));
        m_jPriceTax.setEnabled(!app.getAppUserView().getUser().hasPermission("com.sysfore.pos.purchaseorder.JGoodsReturn"));

        setTxtFields(oLine.getTaxid(), oLine.getTaxes(), JGoodsReturn.m_jSubtotalEuros1.getText(), JGoodsReturn.m_jTotalEuros1.getText());
    ...
    private void setTxtFields(String taxid, String taxesCat, String subtotal, String total) {
        ...
        try {
            ...
            Logger.getLogger(JGRRProductEditor.class.getName()).log(Level.SEVERE, null, ex);
        }
        try {
           taxInfo = tInfo.get(0);
        ...
        m_jSubtotal.setText(JGoodsReturn.m_jSubtotalEuros1.getText());
        m_jTotal.setText(JGoodsReturn.m_jTotalEuros1.getText());
    }
    ...
    public static GoodsReturnLine showMessage(Component parent, AppView app, GoodsReturnLine oLine, int tablePos) throws BasicException {

        Window window = getWindow(parent);

        JGRRProductEditor myMsg;
        if (window instanceof Frame) {
            myMsg = new JGRRProductEditor((Frame) window, true);
        } else {
            myMsg = new JGRRProductEditor((Dialog) window, true);
        }
