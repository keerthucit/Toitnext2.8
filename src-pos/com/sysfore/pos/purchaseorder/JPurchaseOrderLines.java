 ...
 * @author mateen
 */
public class JPurchaseOrderLines extends javax.swing.JPanel {
    ...
    public JPurchaseOrderLines() {
        ...
        columns.addColumn(c);
        c = new TableColumn(1, 190, new DataCellRenderer(javax.swing.SwingConstants.LEFT), new DefaultCellEditor(new JTextField()));
        ...
        columns.addColumn(c);
        c = new TableColumn(5, 56, new DataCellRenderer(javax.swing.SwingConstants.CENTER), new DefaultCellEditor(new JTextField()));
    ...
    public void addLine(PurchaseOrderLine i) {
        m_inventorylines.addRow(i);

        setSelectedIndex(m_inventorylines.getRowCount() - 1);
    }

    public void updateLine(PurchaseOrderLine i, int row) {
    ...
    public void setLine(int index, PurchaseOrderLine i) {
        m_inventorylines.setRow(index, i);
        setSelectedIndex(index);
    }

    public PurchaseOrderLine getLine(int index) {
    ...
    public static void setMultiplychanged(boolean multiplychanged) {
        ...
        double taxtotal = 0.0;
        if (multiplychanged) {
            java.util.List<PurchaseOrderLine> lines = m_inventorylines.getLines();
            ...
            JPurchaseOrder.setChangesmade(multiplychanged);
            JPurchaseOrder.setTotals(total, subtotal, taxtotal);
    ...
    private static class InventoryTableModel extends AbstractTableModel {
        ...
        private ArrayList<PurchaseOrderLine> m_rows = new ArrayList<PurchaseOrderLine>();
        ...
        public Object getValueAt(int row, int column) {
            ...
            PurchaseOrderLine i = m_rows.get(row);
            switch (column) {
        ...
        public PurchaseOrderLine getRow(int index) {
            return m_rows.get(index);
        }

        public void setRow(int index, PurchaseOrderLine oLine) {

            m_rows.set(index, oLine);
            fireTableRowsUpdated(index, index);
        }

        public void addRow(PurchaseOrderLine oLine) {

            insertRow(m_rows.size(), oLine);
        }

        public void insertRow(int index, PurchaseOrderLine oLine) {
    ...
    private static void removeRowFromPanelAndDatabase(int row) {
        PurchaseOrderLine line = m_inventorylines.getRow(row);
        JPurchaseOrder.removeLineFromDB(line);
    }
