 ...
 * @author mateen
 */
public class JGoodsReceipts extends JPanel implements JPanelView {
    ...
    private com.sysfore.pos.purchaseorder.JGoodsReceiptsLines m_polines;
    ...
    private java.util.List<GoodsReceiptsInfo> list;
     ...
     */
    public JGoodsReceipts(AppView app) {
    ...
    //    m_LocationsModelDes = new ComboBoxValModel();
        m_polines = new JGoodsReceiptsLines();
    ...
    private static void deleteGRNLine(GoodsReceiptsLine line) {
    ...
    public String getTitle() {
        return AppLocal.getIntString("Menu.GRN");
    }
...
public void setGoodsReceiptCount(){
    
    int processCount = 0;
     processCount = ProcessInfo.setProcessCount("Goods Receipts", m_dlSales);
        if(processCount>=10){
    ...
    public boolean deactivate() {
        ...
        if (isChangesmade()) {
            int res = JOptionPane.showConfirmDialog(this, "Changes has been made to Current Goods Receipts, Do you want to save changes");
            if (res == 0) {
                retVal = saveData();
    ...
    private void GoodsReceiptsActionSave() throws BasicException {
        ...
        if (status.equals("Draft")) {
            ...
            List<GoodsReceiptsLine> lines1 = m_polines.getLines();
            if (isInsert()) {
                //insert
                String f_Poid = UUID.randomUUID().toString();
                f_Poid = f_Poid.replaceAll("-", "");
                String user = m_App.getAppUserView().getUser().getId();
                m_dlPOReceipts.insertGoodsReceipts(f_Poid, getDocumentNo(), vendor, dateCreate1, dateDelivered1, status, getTotalNotNull(dTotal), getSubTotalNotNull(dSubTotal), getTaxNotNull(tax), user,warehouse);
                enableBothButtons(true);
                     //   ProcessInfo.setProcessInfo("Purchase Order",m_dlSales);
                   //     setPurchaseCount();
                for (GoodsReceiptsLine l : lines1) {
                    m_dlPOReceipts.insertGRNLines(f_Poid, l.getProductID(), l.getMultiply(), l.getPrice(), l.getTaxes(), l.getTaxid());
                }
            } else {
                //update
                String grnId = list.get(index).getId();
                m_dlPOReceipts.updateGRN(grnId, getDocumentNo(), vendor, dateCreate1, dateDelivered1, status, getTotalNotNull(dTotal), getSubTotalNotNull(dSubTotal), tax,warehouse);
                for (GoodsReceiptsLine l : lines1) {
                    // IF PO ALREADY CONTAINS THIS PRODUCT THEN UPDATE ELSE INSERT
                    java.util.List<GoodsReceiptsProductInfo> infoLines = m_dlPOReceipts.getGRNLinesByPoidAndProduct(grnId, l.getProductID());
                    if (infoLines.size() > 0) {
                        if (infoLines.get(0).getPoid() != null) {
                            m_dlPOReceipts.updateGRNLines(grnId, l.getProductID(), l.getMultiply(), l.getPrice(), l.getTaxes());
                        }
                    } else {
    ...
    private String generateDocumentNoFromDB() throws NumberFormatException {
        ...
        String StoreName;
        try {
            docNo = m_dlPOReceipts.getGRNDocumentNo().list().get(0).toString();
    ...
    private void GRNActionComplete() throws BasicException {
        ...
        //update
        m_dlPOReceipts.updateGRN(grnId, getDocumentNo(), vendor, getFormattedDate(dateCreate), getFormattedDate(dateDelivered), status, dTotalT, dSubTotal, tax,warehouse);
        List<GoodsReceiptsLine> lines1 = m_polines.getLines();
        for (GoodsReceiptsLine l : lines1) {
            // IF PO ALREADY CONTAINS THIS PRODUCT THEN UPDATE ELSE INSERT
            java.util.List<GoodsReceiptsProductInfo> infoLines = m_dlPOReceipts.getGRNLinesByPoidAndProduct(grnId, l.getProductID());
        ...
        //setInsert(true);
 m_dlPOReceipts.updateGoodsReceipts(lines1,warehouse);
    ...
    public static void setProductTotalTaxes(Double productTotalTaxes) {
        JGoodsReceipts.productTotalTaxes = productTotalTaxes;
    }
    ...
    public static void setProductSubTotal(Double productSubTotal) {
        JGoodsReceipts.productSubTotal = productSubTotal;
    }
    ...
    public static void setProductTotal(Double productTotal) {
        JGoodsReceipts.productTotal = productTotal;
    }
    ...
    public static void removeLineFromDB(GoodsReceiptsLine line) {
        deleteGRNLine(line);
    }
    ...
    private void m_jDeleteActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_m_jDeleteActionPerformed
        ...
        if (index > -1) {
            GoodsReceiptsLine line = m_polines.getLine(index);
            deleteGRNLine(line);
...
private void jEditAttributesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jEditAttributesActionPerformed
    ...
    ProductInfoExt prod = JPurchaseProductFinder.showMessage(JGoodsReceipts.this, m_dlSales);
    Double rate = 0.0;
    ...
    if (prod != null) {
        ...
        if (productAlreadyExists(prod)) {
            GoodsReceiptsLine i = m_polines.getLine(getProductMatchIndex());
            i.setMultiply(i.getMultiply() + 1);
            m_polines.updateLine(i, getProductMatchIndex());
        } else {
            m_polines.addLine(new GoodsReceiptsLine(prod, prod.getMultiply(), prod.getPriceBuy(), productTaxCatId));
       //     lines.add(prod);
        }
        setChangesmade(true);
                        ...
                        if (isBillAddressValidated()) {
                            if (isShiftAddressValidated()) {
                                if (m_polines.getCount() > 0) {
                                    // TODO add your handling code here:
                                    GRNActionComplete();
                                    showMessageGreen(this, "Goods Receipts completed Successfully");
                                    setChangesmade(false);
                                } else {
...
private void txtShipAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_txtShipAddActionPerformed
    ...
    private void m_jEditLineActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_m_jEditLineActionPerformed
        ...
        if (i < 0) {
            ...
            try {
                JGRNProductEditor.showMessage(this, m_App, m_polines.getLine(i), i);
...
private void printReport(String resourcefile, GoodsReceiptsInfo grn) {
 ...
 String store=m_App.getProperties().getStoreName();
        GoodsReceiptsInfo purchase = new GoodsReceiptsInfo();
         List<GoodsReceiptsLine> lines = m_polines.getLines();
    ...
    private void deleteAction() {
        ...
        if (index > -1) {
            if (list.get(index).getStatus().equals("Draft")) {
                int res = JOptionPane.showConfirmDialog(this, "Are sure you want to delete this Goods Receipts?");
                if (res == 0) {
                    try {
                        m_dlPOReceipts.deleteGRNLines(list.get(index).getId());
                        m_dlPOReceipts.deleteGRN(list.get(index).getId());
                    } catch (BasicException ex) {
                        Logger.getLogger(JGoodsReceipts.class.getName()).log(Level.SEVERE, null, ex);
                    }
                    try {
                ...
                showMessageGreen(this, "Completed Goods Receipts cannot be cancelled");
...
//        }
    ...
    private void setPOLines(String doc, String selectedPOStatus) {
        ...
        for (Iterator<GoodsReceiptsProductInfo> it = poLines.iterator(); it.hasNext();) {
            ...
            m_polines.addLine(new GoodsReceiptsLine(pInfoExt));
        }
    }
    ...
    public static void setChangesmade(boolean changesmade) {
        JGoodsReceipts.changesmade = changesmade;
    }
    ...
    private boolean saveData() {
        try {
            ...
            String status = txtStatus.getSelectedItem().toString();
           String warehouse=whLines.get(jComboWarehouse.getSelectedIndex()-1).getId();
           m_dlPOReceipts.updateGRN(grnId, documentNo, vendor, getFormattedDate(dateCreate), getFormattedDate(dateDelivered), status, dTotal, dSubTotal, tax,warehouse);
    ...
    private boolean productAlreadyExists(ProductInfoExt prod) {
        ...
        boolean retVal = false;
        java.util.List<GoodsReceiptsLine> l = m_polines.getLines();
    ...
    private void setTotalTxtValuesWithoutClearingAddresses() {
        ...
        java.util.List<GoodsReceiptsLine> totalLine = m_polines.getLines();
        double subtotal = 0;
        double tax1 = 0;
        if (totalLine.size() > 0) {
...
//            double producttotal1 = getProductSubTotal() + totaltaxes1;
    ...
    private boolean isWarehouseValidated() {
        boolean retVal = false;
        if (jComboWarehouse.getSelectedIndex() >= 0 || !((jComboWarehouse.getSelectedItem().toString()).equals(null) )) {
            retVal = true;
        }
        return retVal;
    }
    ...
    private void showMessage(JGoodsReceipts aThis, String msg) {
...
private void showMessageGreen(JGoodsReceipts aThis, String msg) {
     //   JOptionPane.showMessageDialog(aThis, msg);
...
private JPanel getLabelGreenPanel(String msg) {
  ...
  //  panel.setBackground(Color.BLUE);
    ...
    private void closePOAction() {
        int index = jListDocumentNo.getSelectedIndex();
        if (index > -1) {
            int res = JOptionPane.showConfirmDialog(this, "Are sure you want to close this Goods Receipts");
            if (res == 0) {
                try {
                    m_dlPOReceipts.closeGRN(list.get(index).getId());
                } catch (BasicException ex) {
                    Logger.getLogger(JGoodsReceipts.class.getName()).log(Level.SEVERE, null, ex);
                }
                try {
